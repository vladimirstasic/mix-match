...
export function ManualTracklist({ onCreated, onBack }: Props) {
  ...
  const save = async () => {
    if (!mixTitle.trim()) {
      setError('Mix title required');
      return;
    }
    if (tracks.some(t => !t.artist.trim() || !t.title.trim())) {
      setError('All tracks need artist and title');
      return;
    }
    ...
    try {
      const payload = {
        ...
        tracks: tracks.map((t, i) => {
          ...
          const endSec = nextTrack
            ? parseInt(nextTrack.startMin) * 60 + parseInt(nextTrack.startSec || '0')
            : startSec + 300;
          return {
            artist: t.artist,
            title: t.title,
        ...
        <Card key={i}>
          <CardContent className="py-3 flex items-center gap-2">
            <div className="flex gap-1 items-center shrink-0">
              <input
                value={track.startMin}
                onChange={e => updateTrack(i, 'startMin', e.target.value)}
                className="w-10 px-1 py-1 text-center text-xs rounded border border-input bg-background"
                placeholder="0"
              />
              <span className="text-xs text-muted-foreground">:</span>
              <input
                value={track.startSec}
                onChange={e => updateTrack(i, 'startSec', e.target.value)}
                className="w-10 px-1 py-1 text-center text-xs rounded border border-input bg-background"
                placeholder="00"
              />
            </div>
            ...
            />
            {tracks.length > 1 && (
              <Button variant="ghost" size="sm" onClick={() => removeTrack(i)}>
