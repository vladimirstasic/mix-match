  ...
  return (
    <div className="space-y-6">
          ...
          <React.Fragment key={seg.id}>
              ...
              className={`border-l-4 transition-shadow ${
                seg.status === 'identified'
                  ? bookmarkedIds.has(seg.id)
                    ? 'border-l-green-500 ring-1 ring-green-500/20 bg-green-500/5'
                    : 'border-l-green-500'
                  : seg.status === 'retrying'
                    ? 'border-l-yellow-500'
            ...
            >
              <CardContent className="py-3 space-y-2">
                ...
                {seg.status === 'identified' && editingId !== seg.id && (
                  <div className="flex items-center gap-1 ml-[100px] sm:ml-[120px] flex-wrap">
                    ...
                    <div className="flex items-center gap-0.5 ml-auto">
                      ...
                      <button
                        className={`p-1 rounded transition-colors ${bookmarkedIds.has(seg.id) ? 'text-blue-500' : 'text-muted-foreground/40 hover:text-blue-500'}`}
                ...
                {seg.attempts > 1 && (
                  <span className="text-xs text-muted-foreground ml-auto">attempt {seg.attempts}</span>
                )}
              </CardContent>
      ...
      <div className="flex flex-wrap items-center gap-2 pt-4 border-t">
        ...
        <Button variant="outline" size="sm" onClick={() => openExport('text')}>
          Text
        </Button>
        <Button variant="outline" size="sm" onClick={() => openExport('mixcloud')}>
          Mixcloud
        </Button>
        <Button variant="outline" size="sm" onClick={() => openExport('soundcloud')}>
          SoundCloud
        </Button>
        <Button variant="outline" size="sm" onClick={() => openExport('youtube')}>
          YouTube
        </Button>
        <Button variant="outline" size="sm" onClick={() => openExport('markdown')}>
          Markdown
        </Button>
