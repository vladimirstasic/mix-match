...
retryRouter.post("/analysis/:id/segments/:segmentId/retry", async (req, res) => {
  ...
  const [segment] = await db.select().from(segments)
    .where(and(eq(segments.id, segmentId), eq(segments.analysisId, id))).limit(1);
  ...
  const job = await retryQueue.add("retry-segment", {
    analysisId: id, segmentId, startSec: segment.startSec, endSec: segment.endSec,
...
retryRouter.post("/analysis/:id/retry-unknown", async (req, res) => {
  ...
  const unknownSegments = await db.select().from(segments)
  ...
  const job = await retryQueue.add("retry-all-unknown", {
    analysisId: id,
    segments: unknownSegments.map((s) => ({ segmentId: s.id, startSec: s.startSec, endSec: s.endSec, attempt: s.attempts + 1 })),
