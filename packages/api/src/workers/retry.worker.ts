...
function getRetryParams(attempt: number): { offsetSec: number; durationSec: number } {
  switch (attempt) {
    case 2:
      return { offsetSec: 7, durationSec: 15 };
    case 3:
      return { offsetSec: 0, durationSec: 10 };
    case 4:
      return { offsetSec: 5, durationSec: 10 };
    default:
      return { offsetSec: 3, durationSec: 12 };
  }
}
    ...
    if (job.name === 'retry-segment') {
      ...
      for (let i = 0; i < data.segments.length; i++) {
        ...
        try {
          ...
          if (err instanceof RateLimitError) {
            console.log(`[retry] Rate limit hit at segment ${i + 1}/${data.segments.length}`);
            break;
          }
        }
